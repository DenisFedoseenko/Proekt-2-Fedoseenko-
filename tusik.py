...
def show_start_screen():
    ...
    text = font.render("Мир столкновений", True, GREEN)
    start_button = font.render("Нажмите 'Пробел', чтобы начать", True, GREEN)
    settings_button = font.render("Нажмите 'Tab', чтобы настройки", True, GREEN)
    screen.fill(WHITE)
...
def show_end_screen(score):
    ...
    font = pygame.font.Font(None, 74)
    text = font.render(f"Игра окончена! Счет: {score}", True, GREEN)
    restart_button = font.render("Нажмите 'Пробел', чтобы начать снова", True, GREEN)
    exit_button = font.render("Нажмите 'ESC', чтобы выход", True, GREEN)
    screen.fill(WHITE)
    screen.blit(text, (SCREEN_WIDTH // 2 - text.get_width() // 2, SCREEN_HEIGHT // 2 - 50))
    ...
    while len(bricks) < BRICKS_PER_LEVEL:
        ...
        if not any(new_brick.colliderect(brick) for brick in bricks) and \
           not any(abs(brick.y - new_brick.y) < BRICK_HEIGHT and
                   (new_brick.x < brick.x + BRICK_WIDTH and new_brick.x + BRICK_WIDTH > brick.x) for brick in bricks):
...
def settings_screen():
    ...
    while waiting:
        ...
        exit_button = font.render("Нажмите 'Shift', чтобы вернуться", True, GREEN)
        ...
        # Ползунки громкости
        pygame.draw.rect(screen, GREEN, (50, 200, 300, 20))
        pygame.draw.rect(screen, GREEN, (50, 300, 300, 20))
...
def game_loop():
    play_music("u.mp3")  # Игровая музыка
